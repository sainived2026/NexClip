"""02 — Ghost Karaoke Renderer
Faded ghost words (rgba 255,255,255,0.35), solid white active with scale 1.05.
"""
...
class GhostKaraokeRenderer(BaseCaptionRenderer):
    def _draw_with_pillow(self, draw: ImageDraw.ImageDraw, canvas: Image.Image,
        ...
        for i, part in enumerate(words):
            ...
            if is_active:
                # Scale 1.05 — use a slightly larger font
                active_font = self._get_font(int(font_size * 1.05))
                color = (255, 255, 255, 255)
                wb = draw.textbbox((0, 0), word_text, font=active_font)
                ww = wb[2] - wb[0]
                # Adjust vertical position for bigger font
                active_h = wb[3] - wb[1]
                draw.text((x_pos, text_y - active_h), word_text, font=active_font, fill=color)
            else:
                # Ghost — 35% opacity white
